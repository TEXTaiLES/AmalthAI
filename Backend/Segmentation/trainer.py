...
class Trainer(BaseTrainer):
    def __init__(self, model, loss, resume, config, train_loader, val_loader=None, train_logger=None, prefetch=True):
        ...
        self.save_dir_custom_1 = self.save_dir_custom # GET SAVE PATH
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            # Prepare a file contains miou value for this experiment run
            print(f"\n meaniou={np.round(mIoU, 3)}\n")
            with open(f"{self.save_dir_custom_1}/result.txt", "w") as f:
                f.write(f"{np.round(mIoU, 3)}\n")
