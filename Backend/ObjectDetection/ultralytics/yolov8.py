...
# Load a model
model = YOLO("yolov8n.pt" if args.transfer_learning == "true" else "yolov8n.yaml") 
...
# Start training
results = model.train(data= args.dataset, epochs=args.epochs, batch = args.bs,lr0 = args.lr, imgsz=460, device = [0],project=f"/yolosave/runs/{args.timestamp}/YOLOv8", name=f"run_{current_time_micro}", degrees = rotate_value, fliplr = flip_value, scale = scale_value)
