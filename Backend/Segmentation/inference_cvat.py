...
def main():
    ...
    def update_path(d, keys, file_key="data_dir"):
        for key in keys:
            if key in d and file_key in d[key]["args"]:
                old_path = d[key]["args"][file_key]
                new_path = new_root + old_path.split("/segm_datasets")[-1]
                d[key]["args"][file_key] = new_path

    # update data_dir to match inference standards
    update_path(config, ["train_loader", "val_loader"], file_key="data_dir")

    # update palette_file to match inference standards
    update_path(config, ["train_loader", "val_loader"], file_key="palette_file")

    # Dataset used for training the model
    dataset_type = config['train_loader']['type']
    assert dataset_type in ['VOC', 'COCO', 'CityScapes', 'ADE20K', 'DeepScene','CVATBased']
