...
class BaseTrainer:
    def __init__(self, model, loss, resume, config, train_loader, val_loader=None, train_logger=None):
        ...
        current_run_path = self.config['trainer']['localpath']
        model_name = self.config['name']
        base_save_dir = self.config['trainer']['save_dir'] 
        new_save_dir = os.path.join(base_save_dir, current_run_path, model_name, f"run_{current_time_micro}")
        self.save_dir_custom = new_save_dir
        start_time = datetime.datetime.now().strftime('%m-%d_%H-%M')
        self.checkpoint_dir = os.path.join(new_save_dir)
        helpers.dir_exists(self.checkpoint_dir)
        config_save_path = os.path.join(self.checkpoint_dir, 'config.json')
        with open(config_save_path, 'w') as handle:
            json.dump(self.config, handle, indent=4, sort_keys=True)

        writer_dir = os.path.join(new_save_dir) # use one directory for config,weights and logs
