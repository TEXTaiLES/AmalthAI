...
# Load a model
model = YOLO("yolo26n.pt" if args.transfer_learning == "true" else "yolo26n.yaml")  # build from YAML and transfer weights
...
# Start training
results = model.train(data= args.dataset, epochs=args.epochs, batch = args.bs,lr0 = args.lr, imgsz=460, device = [0],project=f"/yolosave/runs/{args.timestamp}/YOLO26", name=f"run_{current_time_micro}", degrees = rotate_value, fliplr = flip_value, scale = scale_value)

df = pd.read_csv(f"/yolosave/runs/{args.timestamp}/YOLO26/run_{current_time_micro}/results.csv")

# Locate and print the required metric to help katib
map5095 = df["metrics/mAP50-95(B)"].iloc[-1]
print(f"map5095={map5095:.4f}")

result_path = f"/yolosave/runs/{args.timestamp}/YOLO26/run_{current_time_micro}/result.txt"
